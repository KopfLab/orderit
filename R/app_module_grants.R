...
module_grants_server <- function(input, output, session, data) {
  ...
  get_grants <- reactive({
    validate(
      ...
      need(data$admins$get_data(), "something went wrong retrieving the data"),
    )
    ...
    df <- data$grants$get_data() |>
      ...
      dplyr::left_join(
        data$admins$get_data() |>
          dplyr::rename_with(~ paste0("approver_", .x), dplyr::everything()),
        by = "approver_user_id"
      ) |>
      dplyr::left_join(
        data$admins$get_data() |>
          dplyr::rename_with(~ paste0("orderer_", .x), dplyr::everything()),
        by = "orderer_user_id"
      ) |>
  ...
  grants <- callModule(
    ...
    available_columns = list(
      ...
      `Orderer` = paste(
        orderer_first_name %then% "",
        orderer_last_name %then% ""
      ),
      `Needs approval` = dplyr::case_when(
        is.na(approval_cutoff) ~ "no",
        approval_cutoff == 0 ~ "always",
        TRUE ~ paste(">", scales::label_dollar()(approval_cutoff))
      ),
      `Approver` = paste(
        approver_first_name %then% "",
        approver_last_name %then% ""
      )
    ),
    allow_view_all = TRUE,
  ...
  dialog_inputs <- reactive({
    ...
    admins <- data$admins$get_data() |>
      ...
      tibble::deframe()
    admins <- c(c("Select user" = ""), admins)
    tagList(
      checkboxInput(ns("status"), "Active"),
      selectInput(ns("group"), "Group", choices = c()),
      textInput(ns("name"), "Name"),
      textInput(ns("identifier"), "Speed Type"),
      selectInput(ns("orderer_user_id"), "Orderer", choices = admins),
      ...
      conditionalPanel(
        ...
        selectInput(ns("approver_user_id"), "Approver", choices = admins),
        numericInput(
          ns("approval_cutoff"),
          "Orders over this $ amount require approval",
          value = 0,
          min = 0,
          step = 100
        )
  ...
  observeEvent(input$add, {
    ...
    updateSelectInput(
      inputId = "group",
      choices = data$get_active_user_data()$groups,
      selected = data$get_active_user_data()$groups[1]
    )
    updateCheckboxInput(inputId = "status", value = TRUE)
    updateTextInput(inputId = "name", placeholder = "Enter name of new grant")
    updateTextInput(
      inputId = "identifier",
      placeholder = "Enter speedtype for new grant"
    )
  ...
  observeEvent(input$edit, {
    ...
    updateSelectInput(
      inputId = "orderer_user_id",
      selected = grant$orderer_user_id
    )
    updateCheckboxInput(
      inputId = "needs_approval",
      value = !is.na(grant$approver_user_id)
    )
    updateSelectInput(
      inputId = "approver_user_id",
      selected = grant$approver_user_id
    )
    updateNumericInput(
      inputId = "approval_cutoff",
      value = grant$approval_cutoff
    )
  ...
  check_inputs <- function() {
    ...
    # input checks
    if (
      input$needs_approval &&
        (is.na(input$approval_cutoff) || input$approval_cutoff < 0)
    ) {
      log_warning(
        ns = ns,
        "invalid approval cutoff",
        user_msg = "Approval cutoff: please enter a valid number for the approval cutoff"
      )
      ok <- FALSE
    }
    ...
    if (input$needs_approval && nchar(input$approver_user_id) == 0) {
      log_warning(
        ...
        user_msg = "Approver: please select someone who will approve orders for this grant (or de-select the needs approval option)"
      )
      ok <- FALSE
    }
    ...
    if (nchar(input$orderer_user_id) == 0) {
      log_warning(
        ns = ns,
        "no orderer selected",
        user_msg = "Orderer: please select someone who will place the orders for this grant"
      )
      ok <- FALSE
    }
    ...
    if (nchar(input$name) == 0) {
      log_warning(
        ns = ns,
        "no name provided",
        user_msg = "Name: please enter a name for the grant"
      )
      ok <- FALSE
    }
    return(ok)
  ...
  observeEvent(input$save, {
    ...
    if (check_inputs()) {
      ...
      data$grants$update(
        ...
        approver_user_id = if (input$needs_approval) {
          input$approver_user_id
        } else {
          NA_character_
        },
        approval_cutoff = if (input$needs_approval) {
          input$approval_cutoff
        } else {
          NA_real_
        }
      )
